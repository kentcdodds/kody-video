...
export class KvExportOverlay extends KvElement {
    ...
    render() {
        ...
        this.replaceChildren(h('div', { className: 'export-overlay', role: 'dialog', 'aria-label': 'Exporting video' }, h('div', { className: 'export-overlay-stage' }, canvas), h('div', { className: 'export-overlay-info' }, h('h2', null, 'Exporting your video…'), h('p', { className: 'muted' }, `${projectName} — keep the app open`), h('div', { className: 'progress-bar', 'aria-label': 'Export progress' }, this.#bar), this.#percentLabel)));
        this.setProgress(this.props.progress ?? 0);
    }
    /** Imperative: progress ticks many times a second — no re-render. */
    setProgress(ratio) {
        if (!this.#bar || !this.#percentLabel)
            return;
