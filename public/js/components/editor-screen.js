...
import { duplicateSelectedClip, moveSelectedClip, removeClip, trimClip, undoLastDelete, } from "../lib/project-actions.js";
import { refineClipFilmstrip, THUMB_COUNT } from "../lib/thumbs.js";
import { effectiveDurationMs, formatDuration, } from "../lib/types.js";
import { iconBack, iconChevronLeft, iconChevronRight, iconDuplicate, iconPlay, iconTrash, iconTrim, iconUndo, } from "./icons.js";
import { KvClipPreview } from "./clip-preview.js";
import { KvTimeline } from "./timeline.js";
import { KvTrimStrip } from "./trim-strip.js";
export class KvEditorScreen extends KvElement {
    #selectedClipId = null;
    #trimming = false;
    #preview = null;
    #previewKey = null;
    #refineAttempted = new Set();
    els = {};
    update() {
        this.sync();
    }
            ...
            this.props.showToast('Clip deleted', {
                ...
                onAction: () => {
                    void (async () => {
                        ...
                        if (restored)
                            this.#setSelectedClipId(restored.id);
                        this.props.refresh();
    ...
    onWindowKeyDown = (event) => {
        ...
        switch (event.code) {
            ...
            case 'ArrowRight': {
                ...
                const nextIndex = Math.min(clips.length - 1, Math.max(0, (selectedIndex < 0 ? clips.length - 1 : selectedIndex) + step));
    ...
    #refineFilmstrips() {
        ...
        for (const id of this.#refineAttempted) {
            if (!ids.has(id))
                this.#refineAttempted.delete(id);
        }
    ...
    render() {
        ...
        els.meta = h('small');
        els.playTop = h('button', {
            type: 'button',
            className: 'btn-icon',
            'aria-label': 'Play project preview',
            onclick: () => onPlay(),
        }, iconPlay());
        els.top = h('div', { className: 'editor-top' }, h('button', {
            type: 'button',
            className: 'btn-icon',
            'aria-label': 'Back to camera',
            onclick: () => onOpenCamera(),
        }, iconBack()), h('div', { className: 'editor-meta' }, h('strong', null, project.name), els.meta), els.playTop);
        ...
        const action = (label, icon, onclick, { ariaLabel, prominent, danger, } = {}) => {
            const button = h('button', {
                type: 'button',
                className: `editor-action${prominent ? ' prominent' : ''}${danger ? ' danger' : ''}`,
                'aria-label': ariaLabel ?? label,
                onclick,
            }, h('span', { className: 'editor-action-icon' }, icon), h('span', { className: 'editor-action-label' }, label));
            return button;
        };
        ...
        els.trimAction = action('Trim', iconTrim(), () => {
            this.#preview?.pause();
            this.#setTrimming(true);
        }, { prominent: true });
        ...
        els.actions = h('div', { className: 'editor-actions', role: 'toolbar', 'aria-label': 'Clip actions' }, els.deleteAction, els.duplicateAction, els.trimAction, els.leftAction, els.rightAction);
        els.undoButton = h('button', {
            type: 'button',
        ...
        }, iconUndo(18), ' Undo delete');
        els.goButton = h('button', { type: 'button', className: 'go-button compact', onclick: () => onOpenExport() }, 'Go');
        ...
        els.panel = h('div', { className: 'editor-panel' }, els.panelStrip, els.actions, els.toolbar, els.keyHints);
    ...
    sync() {
        ...
        if (selected) {
            ...
            this.#preview = null;
            els.stage.replaceChildren(h('div', { className: 'editor-empty-preview' }, 'Select a clip in the timeline'));
        }
