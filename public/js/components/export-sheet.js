...
function formatFileInfo(ext, bytes) {
  ...
  if (bytes === null || bytes === undefined) return type
...
export class KvExportSheet extends KvElement {
  ...
  renderContent() {
    ...
    const btn = (className, label, onclick, disabled = busy) =>
      h('button', { type: 'button', className, disabled, onclick }, label)
    const linkBtn = (label, onclick, disabled = busy) =>
      ...
      this.#dialog.replaceChildren(
        ...
        watermarked && !purchased
          ? h(
              ...
              linkBtn('Get Plus — $0.99 removes it & unlocks 6 projects', () => onRemoveWatermark(), false),
              ' · ',
