...
export class KvTimeline extends KvElement {
    ...
    render() {
        ...
        if (clips.length === 0) {
            ...
            this.#signature = null;
            this.#positioned = false;
            this.replaceChildren(h('div', { className: 'timeline', 'aria-label': 'Timeline empty' }, h('p', { className: 'timeline-empty muted' }, 'Hold the preview to add clips')));
            return;
        }
        if (!this.#track) {
            this.#track = h('div', {
    ...
    #buildTiles(clips) {
        ...
        const slots = clips.map((clip, index) => {
            ...
            const tile = h('button', {
            ...
            }, h('div', { className: 'clip-filmstrip' }, thumbs.length > 0
                ...
                : h('div', { className: 'clip-filmstrip-placeholder', 'aria-hidden': 'true' })), h('span', { className: 'clip-dur' }, formatDuration(effectiveDurationMs(clip))));
    ...
    #beginLift(clipId) {
        ...
        if (!drag || drag.clipId !== clipId || drag.lifted || drag.scrolling)
            return;
        drag.lifted = true;
    ...
    #onPointerMove(event) {
        ...
        if (!state.scrolling && Math.hypot(dx, dy) < MOVE_CANCEL_PX)
            return;
        ...
        if (this.#track)
            this.#track.scrollLeft = state.startScrollLeft - dx;
