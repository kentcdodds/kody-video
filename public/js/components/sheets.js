...
export class KvConfirmSheet extends KvElement {
    render() {
        const { title, message, confirmLabel = 'Delete', cancelLabel = 'Cancel', onConfirm, onClose, } = this.props;
        let busy = false;
        ...
        dialog.replaceChildren(h('h3', null, title), h('p', { className: 'sheet-lede muted' }, message), 
...
export class KvRenameSheet extends KvElement {
    render() {
        const { initialName, title = 'Rename project', confirmLabel = 'Save', onClose, onSave } = this.props;
        let busy = false;
        const dialog = sheetShell(this, {
            label: title,
            onDismiss: onClose,
        ...
        const cancelButton = h('button', { type: 'button', className: 'btn btn-ghost', onclick: () => onClose() }, 'Cancel');
        const saveButton = h('button', { type: 'button', className: 'btn btn-primary', onclick: () => void save() }, confirmLabel);
        const save = async () => {
            busy = true;
        ...
        dialog.replaceChildren(h('h3', null, title), h('div', { className: 'field' }, h('label', { htmlFor: 'project-name' }, 'Name'), input), errorBanner, h('div', { className: 'sheet-actions' }, cancelButton, saveButton));
        input.focus();
        input.select();
    }
}
...
export class KvHomeOptionsSheet extends KvElement {
    render() {
        ...
        const option = (label, onclick, danger = false) => h('button', {
            type: 'button',
            className: `home-option-btn${danger ? ' home-option-danger' : ''}`,
            onclick,
        }, label);
        dialog.replaceChildren(h('h3', null, projectName), h('p', { className: 'sheet-lede muted' }, 'What do you want to do?'), h('div', { className: 'home-options-list' }, option('Open', () => onOpen()), option('Rename', () => onRename()), option('Save backup', () => onBackup()), option('Delete', () => onDelete(), true)), h('div', { className: 'sheet-actions' }, h('button', { type: 'button', className: 'btn btn-ghost', onclick: () => onClose() }, 'Cancel')));
    }
}
...
export class KvUpsellSheet extends KvElement {
    render() {
        ...
        dialog.replaceChildren(h('h3', null, 'Kody Video Plus'), h('p', { className: 'sheet-copy' }, `The free plan includes 1 project. Plus is a one-time $0.99 purchase that unlocks ` +
            `${MAX_PROJECTS} project slots and removes the watermark from exports — forever, on ` +
            `this device and any device you restore it on.`), h('div', { className: 'sheet-actions' }, h('button', { type: 'button', className: 'btn btn-ghost', onclick: () => onClose() }, 'Not now'), h('button', {
            type: 'button',
        ...
        }, 'Get Plus — $0.99')), h('button', { type: 'button', className: 'link-button sheet-footnote', onclick: () => onRestore() }, 'Already paid? Restore your purchase'));
    }
}
...
export class KvRestoreSheet extends KvElement {
    render() {
        ...
        dialog.replaceChildren(h('h3', null, 'Restore purchase'), h('p', { className: 'muted sheet-lede' }, 'Paste the confirmation link from your Stripe receipt email (or the checkout session id ' +
            'starting with “cs_”). We’ll verify it and remove the watermark on this device.'), h('div', { className: 'field' }, h('label', { htmlFor: 'restore-input' }, 'Receipt link or session id'), input), errorBanner, h('div', { className: 'sheet-actions' }, cancelButton, restoreButton));
        input.focus();
    }
}
