...
export class KvTrimStrip extends KvElement {
    #startMs = 0;
    #endMs = 0;
    render() {
        ...
        const track = h('div', { className: 'trim-strip-track' }, h('div', { className: 'trim-strip-filmstrip', 'aria-hidden': 'true' }, thumbs.length > 0
            ? thumbs.map((thumb) => h('img', {
                className: 'trim-strip-frame',
                src: this.blobUrl(thumb),
                alt: '',
                draggable: false,
            }))
            : h('div', { className: 'clip-filmstrip-placeholder' })), dimLeft, dimRight, selection, leftHandle, rightHandle);
        const sync = () => {
        ...
        const cancelButton = h('button', { type: 'button', className: 'btn btn-ghost', onclick: () => onCancel() }, 'Cancel');
        ...
        this.replaceChildren(h('div', { className: 'trim-strip', role: 'group', 'aria-label': 'Trim clip' }, h('div', { className: 'trim-strip-meta' }, keptLabel, errorLabel), track, h('div', { className: 'trim-strip-actions' }, cancelButton, doneButton)));
        sync();
    }
}
