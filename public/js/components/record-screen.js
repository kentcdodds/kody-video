...
import { dragZoomValue } from "../lib/drag-zoom.js";
import { reportError } from "../lib/error-reporting.js";
import { isInteractiveTarget } from "../lib/keyboard.js";
import { getLocationFix } from "../lib/location.js";
import { startMicLevelMonitor } from "../lib/mic-monitor.js";
import { appendRecording, removeClip, undoLastDelete } from "../lib/project-actions.js";
import { HoldRecorder } from "../lib/recorder.js";
import { isScreenRecordingSupported, startScreenRecording, } from "../lib/screen-recorder.js";
import { setLocationTaggingEnabled } from "../lib/storage.js";
import { formatStoragePercent, storageSeverity, } from "../lib/storage-space.js";
import { captureLiveThumbs } from "../lib/thumbs.js";
import { effectiveDurationMs, formatDuration, } from "../lib/types.js";
import { formatZoomLabel, nearestZoomLevel, zoomChipLevels } from "../lib/zoom-chips.js";
import { iconBack, iconDeleteLast, iconEditor, iconFlip, iconLens, iconLocation, iconPlay, iconScreen, iconTimer, iconTorch, } from "./icons.js";
/** Finger tremble tolerance before drag-to-zoom engages. */
const DRAG_ZOOM_DEADZONE_PX = 14;
export class KvRecordScreen extends KvElement {
    ...
    async beginRecord(nextPointerId, nextRecordingMode) {
    ...
    async endRecord(endPointerId) {
        ...
        try {
            ...
            await appendRecording(await this.props.ensureProjectId(), {
                ...result,
    ...
    startSelfTimer() {
        if (this.recording || this.props.interactionLocked || this.countdown !== null)
            return;
        if (this.screenSession)
            return;
    ...
    toggleLocationTagging() {
        if (this.recording || this.countdown !== null)
            return;
        if (this.locationTagging) {
            void (async () => {
                try {
    ...
    onWindowKeyDown = (event) => {
        if (event.repeat || this.props.interactionLocked)
            return;
        ...
        switch (event.code) {
            ...
            case 'KeyP': {
                if (!this.recording && this.props.clips.length > 0)
                    this.props.onPlay();
                return;
            }
            case 'KeyF': {
            ...
            case 'KeyS': {
                ...
                if (this.screenSession)
                    void this.finishScreenRecord();
                else if (isScreenRecordingSupported())
    ...
    render() {
        ...
        els.recordOverlay = h('div', { className: 'record-overlay', hidden: true }, h('div', { className: 'record-pill', 'aria-live': 'polite' }, h('span', { className: 'record-dot' }), els.recordPillLabel, els.recordElapsed));
        els.screenElapsed = h('span', { className: 'record-elapsed' });
        els.screenOverlay = h('div', { className: 'record-overlay', hidden: true }, h('div', { className: 'record-pill', 'aria-live': 'polite' }, h('span', { className: 'record-dot' }), h('span', { className: 'record-pill-label' }, 'SCREEN — TAP TO STOP'), els.screenElapsed));
        els.countdownNumber = h('span', { className: 'countdown-number' });
        els.countdownOverlay = h('div', { className: 'countdown-overlay', 'aria-live': 'assertive', hidden: true }, els.countdownNumber);
        els.holdHint = h('div', { className: 'hold-hint', hidden: true }, h('strong', null, 'Hold anywhere'), h('span', null, 'release to stop'));
        els.zoomHud = h('div', { className: 'zoom-hud', 'aria-hidden': 'true' });
        els.permissionMessage = h('p');
        els.permissionPanel = h('div', { className: 'permission-panel', hidden: true }, h('div', null, h('h2', null, 'Camera access'), els.permissionMessage, h('button', { type: 'button', className: 'btn btn-primary', onclick: () => void camera.start() }, 'Try again')));
        els.stage = h('div', {
        ...
        }, els.video, els.recordOverlay, els.screenOverlay, els.countdownOverlay, els.holdHint, els.zoomHud, els.permissionPanel);
        // Top bar
        ...
        els.top = h('div', { className: 'record-top' }, h('a', {
            href: '/',
            className: 'btn-icon',
        ...
        }, iconBack()), h('div', { className: 'record-meta' }, els.projectName, els.totalDuration, els.storagePill, els.micBlockedPill, els.micSilentPill), h('div', { className: 'record-top-actions' }, els.screenButton, els.torchButton, els.flipButton));
        els.zoomRow = h('div', {
        ...
        els.timerButton = h('button', {
            type: 'button',
            className: 'btn-icon',
            'aria-label': 'Self-timer',
            onclick: () => this.startSelfTimer(),
        }, iconTimer());
        els.locationButton = h('button', {
            type: 'button',
            className: 'btn-icon',
            'aria-label': 'Toggle location tagging',
            onclick: () => this.toggleLocationTagging(),
        }, iconLocation());
        els.playButton = h('button', {
            type: 'button',
            className: 'btn-icon',
            'aria-label': 'Play project preview',
            onclick: () => onPlay(),
        }, iconPlay());
        els.deleteButton = h('button', {
            type: 'button',
            className: 'btn-icon',
            'aria-label': 'Delete last clip',
            onclick: () => this.deleteLastClip(),
        }, iconDeleteLast());
        els.goButton = h('button', { type: 'button', className: 'go-button', onclick: () => onOpenExport() }, 'Go');
        els.dock = h('div', { className: 'record-dock' }, h('div', { className: 'record-tools' }, els.editorButton, els.timerButton, els.locationButton, els.playButton, els.deleteButton), els.goButton);
        els.root = h('div', { className: 'record-screen' }, els.stage, els.top, els.zoomRow, els.keyHints, els.dock);
    ...
    sync() {
        ...
        if (this.countdown !== null)
            els.countdownNumber.textContent = String(this.countdown);
        els.holdHint.hidden = !(!recording &&
            !this.screenRecording &&
            this.countdown === null &&
            camera.isReady);
        els.holdHint.classList.toggle('hold-hint-subtle', clips.length > 0);
        const needsPermission = camera.permission.status === 'denied' ||
        ...
        els.screenButton.setAttribute('aria-label', this.screenRecording ? 'Stop screen recording' : 'Record your screen');
    ...
    syncZoomChips() {
        ...
        const activeZoomLevel = camera.zoom && zoomLevels.length > 0 ? nearestZoomLevel(zoomLevels, camera.zoom.value) : null;
        ...
        if (signature === this.#chipSignature)
            return;
            ...
            chips.push(h('button', {
                ...
                onclick: (event) => {
                    ...
                    void camera
                        ...
                        if (range)
                            this.showZoomHud(range.value);
                    })
                        .catch(() => undefined);
                },
            }, iconLens(14), `${camera.rearLensIndex + 1}/${camera.rearLensCount}`));
        }
