...
import '../components/export-overlay.js'
import '../components/export-sheet.js'
import '../components/onboarding-overlay.js'
import '../components/playback-overlay.js'
import '../components/record-screen.js'
...
export class KvProjectPage extends KvElement {
  ...
  startExport(options) {
    ...
    const wakeLockState = { current: null, released: false }
    ...
    void (async () => {
      try {
        ...
        reportError(err, 'export', {
          ...
          exportDetail: err && typeof err === 'object' ? err.exportDetail : undefined,
        })
        if (this.exportRun !== runId) return
        this.setExportState({
          status: 'error',
  ...
  syncView() {
    ...
    if (wantedTag !== els.screenTag) {
      ...
      if (wantedTag) {
        els.screen = h(wantedTag, { props: this.#screenProps() })
        els.root.prepend(els.screen)
      }
    } else if (els.screen) {
      els.screen.props = this.#screenProps()
      els.screen.update()
    ...
    if (this.playing && !els.playback) {
      els.playback = h('kv-playback-overlay', {
        props: {
          clips,
          onClose: () => {
            this.playing = false
    ...
    if (exporting && !els.exportOverlay) {
      els.exportOverlay = h('kv-export-overlay', {
        props: {
      ...
      els.root.append(els.exportOverlay)
    ...
    const sheetOpen = this.exportState && this.exportState.status !== 'exporting'
    if (sheetOpen) {
      ...
      if (!els.exportSheet) {
        els.exportSheet = h('kv-export-sheet', { props: sheetProps })
        els.root.append(els.exportSheet)
      } else {
    ...
    if (this.restoring && !els.restoreSheet) {
      els.restoreSheet = h('kv-restore-sheet', {
        props: {
          onClose: () => {
      ...
      els.root.append(els.restoreSheet)
    ...
    if (this.onboardingOpen && !els.onboarding) {
      els.onboarding = h('kv-onboarding-overlay', {
        props: {
          onDismiss: () => {
            void (async () => {
      ...
      els.root.append(els.onboarding)
