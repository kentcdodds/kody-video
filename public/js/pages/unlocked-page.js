...
import { verifyPurchaseSession } from "../lib/entitlement.js";
...
export class KvUnlockedPage extends KvElement {
    result = null;
    mounted() {
        ...
        const body = result === null
            ...
            : result.unlocked
                ? [
                    ...
                    h('p', { className: 'muted' }, 'Thank you for supporting Kody Video. Every export from this device is now ' +
                ...
                : [
                    ...
                    h('p', { className: 'muted' }, `${result.error ?? 'Could not verify this purchase.'} If you paid and keep seeing ` +
                        'this, retry from the link in your Stripe receipt email.'),
                ];
        this.replaceChildren(h('div', { className: 'screen unlocked-screen' }, h('div', { className: 'unlocked-card' }, brandMark({ size: 110, className: 'export-celebrate-art', variant: 'share' }), body, result === null
            ? null
            : h('a', { className: 'btn btn-primary', href: '/' }, result.unlocked ? 'Start creating' : 'Back to Kody Video'))));
    }
}
