...
export async function openCameraStream(facing, options = {}) {
 ...
 */
export async function preferredIosRearCameraId() {
    if (!navigator.mediaDevices?.enumerateDevices)
        return undefined;
    try {
        const devices = await navigator.mediaDevices.enumerateDevices();
        const rear = devices.filter((device) => device.kind === 'videoinput' && /\bback\b/i.test(device.label));
        const byPreference = [
...
export function stopAudioTracks(stream) {
 ...
 */
export async function measureBlobDuration(blob, timeoutMs = 5000) {
...
export async function canFlipCamera() {
    if (!navigator.mediaDevices?.enumerateDevices)
        return false;
    try {
        const devices = await navigator.mediaDevices.enumerateDevices();
...
export async function shareClipFile(clip, projectName, index) {
...
export function projectFilename(projectName, ext = 'webm') {
