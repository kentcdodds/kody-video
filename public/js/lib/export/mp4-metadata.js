 ...
 */
export function injectMp4Metadata(buffer, input) {
    try {
...
function injectMp4MetadataInner(buffer, input) {
    const hasChapters = input.chapters.length > 0;
    const location = input.location ?? null;
    if (!hasChapters && !location)
        return buffer;
    ...
    if (moovIndex !== top.length - 1)
        return buffer;
    const moov = top[moovIndex];
    if (moov.size > 0xffff_ffff - 8)
        return buffer;
    const children = [];
    if (hasChapters)
        children.push(buildChplBox(input.chapters));
    if (location)
        children.push(buildXyzBox(location.lat, location.lng));
    if (children.length === 0)
        return buffer;
    const udta = wrapBox(TYPE_UDTA, concat(children));
    const newMoovSize = moov.size + udta.byteLength;
    if (newMoovSize > 0xffff_ffff)
        return buffer;
 ...
 */
function buildChplBox(chapters) {
    ...
    for (const e of entries)
        payloadLen += 8 + 1 + e.titleBytes.byteLength;
...
function listTopLevelBoxes(bytes) {
    ...
    while (offset + 8 <= bytes.byteLength) {
        ...
        if (size === 1) {
            ...
            if (!Number.isSafeInteger(large) || large < 16)
                break;
            size = large;
            headerSize = 16;
        }
