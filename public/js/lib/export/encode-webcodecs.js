...
export function supportsWebCodecsExport() {
 ...
 */
export async function exportWithWebCodecs(plan, onProgress, getPreviewCanvas, watermarkImage) {
...
async function injectMetadataBestEffort(blob, mimeType, chapters, clipsInPlan) {
...
async function probeOutputSize(plan) {
...
function clipRecordingStartMs(clip) {
    return clip.createdAt - clip.durationMs;
}
function clipsSpanMultipleDays(clips) {
    if (clips.length <= 1)
        return false;
    const days = new Set();
...
function formatChapterTitle(clip, includeDate) {
...
function makeFrameSink({ startSec, endSec, canvas, ctx, videoSource, state, getPreviewCanvas, watermarkImage, }) {
    return (draw, mediaTimeSec, options) => {
 ...
 */
async function pumpSegmentVideoDecoded(args) {
 ...
 */
async function pumpSegmentVideo({ video, ...shared }) {
    ...
    const emitAt = (mediaTimeSec, options) => emit((ctx, width, height) => {
    ...
    try {
        await new Promise((resolve, reject) => {
            ...
            const handleFrame = (mediaTimeSec) => {
                if (finished)
                    return;
 ...
 */
function sliceSegmentAudio(buffer, startMs, segmentMs) {
