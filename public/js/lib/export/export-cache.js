...
export async function withExportCacheReserved(fn) {
...
async function tryExclusive(fn) {
    ...
    if (!locks?.request)
        return fn();
    return locks.request(CACHE_LOCK, { mode: 'exclusive', ifAvailable: true }, async (lock) => lock ? fn() : null);
}
/** Total bytes sitting in the export cache (0 when OPFS is unavailable). */
export async function estimateExportCacheBytes() {
    const entries = await listExportEntries();
    return entries.reduce((sum, entry) => sum + entry.sizeBytes, 0);
}
async function clearLastExportMeta() {
 ...
 */
export async function sweepExportCache() {
    await tryExclusive(async () => {
        ...
        if (settings.lastExport) {
            ...
            const projectExists = projects.some((project) => project.id === settings.lastExport?.projectId);
            if (projectExists) {
