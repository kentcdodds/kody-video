...
async function persistLastExportInner(args) {
    ...
    const adoptable = result.opfsBacked && result.opfsName
    ...
    if (adoptable && result.opfsName) {
        ...
        if (!file || file.size !== result.blob.size)
            return;
 ...
 */
export async function loadMatchingExport(projectId, signature) {
    const settings = await getSettings();
    const last = settings.lastExport;
    if (!last || last.projectId !== projectId || last.signature !== signature)
        return null;
