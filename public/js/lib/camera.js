...
async function openCombinedOrVideoStream(facing, deviceId) {
...
function rememberedRearLens() {
    try {
        const raw = localStorage.getItem(REAR_LENS_STORAGE_KEY);
        if (!raw)
            return null;
        const parsed = JSON.parse(raw);
        if (typeof parsed.id !== 'string' || typeof parsed.index !== 'number')
            return null;
...
function rememberRearLens(lens) {
    try {
        if (lens)
            localStorage.setItem(REAR_LENS_STORAGE_KEY, JSON.stringify(lens));
        else
            localStorage.removeItem(REAR_LENS_STORAGE_KEY);
    }
    catch {
...
async function resolveRearLens() {
    if (isIosBrowser())
        return preferredIosRearCameraId();
    const remembered = rememberedRearLens();
    if (!remembered)
        return undefined;
    const lenses = await listRearCameras();
    if (lenses.includes(remembered.id))
 ...
 */
export function createCamera(notify) {
    let videoEl = null;
    ...
    let facingIntent = 'environment';
    let startInFlight = null;
    let micInFlight = null;
    let micPrimed = false;
    let rearLenses = [];
    ...
    function attachToVideo(next) {
        if (!videoEl)
            return;
        videoEl.srcObject = next;
        void videoEl.play().catch(() => undefined);
    }
    function readTrackCapabilities(next) {
    ...
    async function syncRearLenses(active) {
        ...
        if (camera.stream !== active || facingIntent !== 'environment')
            return;
        rearLenses = lenses;
    ...
    async function start() {
        ...
        const run = (async () => {
            ...
            try {
                const rememberedLens = facingIntent === 'environment' ? await resolveRearLens() : undefined;
    ...
    async function switchRearLens() {
        const lenses = rearLenses;
        if (facingIntent !== 'environment' || lenses.length < 2)
            return;
        ...
        try {
            ...
            try {
                ...
                rememberRearLens(openedId && openedIndex >= 0 ? { id: openedId, index: openedIndex } : null);
            }
            catch (err) {
    ...
    async function enableMic() {
         ...
         * exists to avoid.
         */
        const reopenCombinedStream = async () => {
            const current = camera.stream;
            if (!current)
        ...
        const attachToCurrentStream = async (attempt = 0) => {
            const current = camera.stream;
            if (!current) {
                throw new Error('Camera not ready');
            }
    ...
    function attachVideo(element, signal) {
        videoEl = element;
