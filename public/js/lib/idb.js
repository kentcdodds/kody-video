...
export function openDB(name, version, { upgrade } = {}) {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            const db = request.result;
            resolve({
                get: (storeName, key) => requestToPromise(db.transaction(storeName).objectStore(storeName).get(key)),
                put: (storeName, value) => requestToPromise(db.transaction(storeName, 'readwrite').objectStore(storeName).put(value)),
                delete: (storeName, key) => requestToPromise(db.transaction(storeName, 'readwrite').objectStore(storeName).delete(key)),
                getAllFromIndex: (storeName, indexName) => requestToPromise(db.transaction(storeName).objectStore(storeName).index(indexName).getAll()),
