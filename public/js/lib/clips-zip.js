...
export async function buildClipsZip(clips) {
    ...
    const createStream = () => makeZip(clips.map((clip, index) => ({
    ...
    const file = await withExportCacheReserved(() => streamToOpfsFile(`clips-${Date.now()}.zip`, createStream()));
    if (file)
