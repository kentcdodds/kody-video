...
import { clearExportCache } from '../lib/export/export-cache.ts'
import { dismissIosInstallHint, shouldShowIosInstallHint } from '../lib/install-hint.ts'
import {
  SHOWCASE_EDITION,
  SHOWCASE_PR_NUMBER,
  SHOWCASE_PR_URL,
...
import { canPromptInstall, promptInstall, subscribeInstallPrompt } from '../lib/install-prompt.ts'
import { downloadBlob, shareOrDownload } from '../lib/media.ts'
import type { HomeLoaderData, ProjectSummary } from '../lib/project-actions.ts'
import { loadHomePage } from '../lib/project-actions.ts'
import {
  BackupFormatError,
  importProjectBackup,
  parseProjectBackup,
...
import { deleteProject, getClipsForProject, renameProject } from '../lib/storage.ts'
import {
  formatBytes,
...
import { FREE_PROJECTS, MAX_PROJECTS, NEW_PROJECT_ID, formatDuration } from '../lib/types.ts'
import { navigate } from '../router.ts'
import { brandMark } from '../components/brand-mark.ts'
import { iconClose, iconLock, iconMore, iconPlus, iconShareIos } from '../components/icons.ts'
import {
  KvConfirmSheet,
  KvHomeOptionsSheet,
  KvRenameSheet,
...
export class KvHomePage extends KvElement<void> {
  data: HomeLoaderData | null = lastHomeData
  error: string | null = null
  busy = false
  notice: string | null = null
  importProgress: string | null = null
  ...
  prefetchedClips: { projectId: string; clips: ReturnType<typeof getClipsForProject> } | null =
    null
  refreshVersion = 0
  ...
  refresh = () => {
    ...
    void loadHomePage()
      .then((loaded) => {
        if (this.signal.aborted || version !== this.refreshVersion) return
        lastHomeData = loaded
        this.data = loaded
        this.error = null
        this.update()
  ...
  backupProject(project: ProjectSummary): void {
    void (async () => {
      this.busy = true
      this.error = null
      this.notice = null
  ...
  #openSheet(element: HTMLElement): void {
  ...
  #openOptions(project: ProjectSummary): void {
  ...
  override render(): void {
      ...
      this.#main.replaceChildren(
        this.error
          ? h(
              ...
              h('div', { className: 'error-banner' }, this.error),
            )
    ...
    const slotNodes = slots.map((project: ProjectSummary | null, index: number) => {
      if (project) {
        return h(
          'article',
          {
          ...
          project.posterThumb
            ...
            : null,
          h('div', { className: 'slot-fade', 'aria-hidden': 'true' }),
          h(
            'a',
            { className: 'slot-open', href: `/project/${project.id}` },
            h('span', { className: 'slot-number' }, `Slot ${index + 1}`),
            h('strong', null, project.name),
            h(
              'small',
              null,
              `${project.clipCount} clip${project.clipCount === 1 ? '' : 's'} · ${formatDuration(project.durationMs)}`,
            ),
          ),
          h(
            'button',
      ...
      if (index < projectLimit) {
        return h(
          ...
          h('span', { className: 'slot-plus', 'aria-hidden': 'true' }, iconPlus(26)),
          h('strong', null, 'New project'),
          h('small', null, `Slot ${index + 1}`),
        )
      }
      return h(
        'button',
        ...
        h('span', { className: 'slot-plus', 'aria-hidden': 'true' }, iconLock(22)),
        h('strong', null, 'Plus slot'),
        h('small', null, 'Unlock with Kody Video Plus'),
      )
    })
    ...
    this.#main.replaceChildren(
      h(
        ...
        h(
          ...
          h(
            'div',
            { className: 'home-hero-art', 'aria-hidden': 'true' },
            brandMark({ size: 96, className: 'brand-hero-art', variant: 'camera' }),
          ),
          h('h1', { className: 'brand' }, 'Kody ', h('span', null, 'Video')),
          h('p', { className: 'lede' }, 'Hold to record. Tap Go to share.'),
        ),

        this.showShowcaseBanner
          ? h(
              'div',
              { className: 'home-migrate home-showcase', role: 'note' },
              h(
                'span',
                null,
                h(
                  ...
                  h('a', { href: 'https://kody.video' }, 'kody.video'),
                  '.',
                ),
                ' Projects stay in this browser per-site, so record over there. Curious how this edition came to be? Read the agent’s analysis in ',
                h(
                  'a',
                  { href: SHOWCASE_PR_URL, target: '_blank', rel: 'noreferrer noopener' },
        ...
        this.error ? h('div', { className: 'error-banner' }, this.error) : null,
        this.notice ? h('p', { className: 'home-notice' }, this.notice) : null,
        this.importProgress
          ? h(
              'p',
              { className: 'home-notice', role: 'status', 'aria-live': 'polite' },
              `${this.importProgress} Keep this tab open.`,
            )
          : null,
        ...
        this.showInstallHint
          ? h(
              ...
              h('span', { className: 'install-hint-icon', 'aria-hidden': 'true' }, iconShareIos(18)),
              h(
                'span',
                null,
                'Install Kody Video: tap ',
                h('strong', null, 'Share'),
                ', then ',
                h('strong', null, 'Add to Home Screen'),
                ' — full screen, and your clips are safer from Safari’s storage cleanup.',
              ),
              h(
                'button',
                {
        ...
        h('section', { className: 'project-slots', 'aria-label': 'Kody Video projects' }, slotNodes),
        ...
        h(
          ...
          atCap && !plus
            ? h(
                'button',
                { type: 'button', className: 'btn btn-primary', onclick: () => this.#openUpsell() },
                'Get more projects',
              )
            : h(
                'button',
