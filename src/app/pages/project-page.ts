...
import { createCamera, type Camera } from '../lib/camera.ts'
import { buildClipsZip } from '../lib/clips-zip.ts'
import { REMOVE_WATERMARK_LINK } from '../lib/entitlement.ts'
import { clearExportMarker, markExportStarted, reportError } from '../lib/error-reporting.ts'
import { exportProject, type ExportResult } from '../lib/export/index.ts'
import { exportSignature, loadMatchingExport, persistLastExport } from '../lib/export/last-export.ts'
import { MediaElementFailureError } from '../lib/export/media-error.ts'
import { wait } from '../lib/export/shared.ts'
import { canShareFile, downloadBlob, isIosBrowser, projectFilename, shareFile } from '../lib/media.ts'
import { loadProjectPage, type ProjectLoaderData } from '../lib/project-actions.ts'
import { createProject, setOnboardingDismissed } from '../lib/storage.ts'
import { requestPersistentStorage } from '../lib/storage-space.ts'
import { NEW_PROJECT_ID, type ProjectId } from '../lib/types.ts'
import { navigate } from '../router.ts'
import { KvEditorScreen } from '../components/editor-screen.ts'
import { KvExportOverlay } from '../components/export-overlay.ts'
import { KvExportSheet, type ExportSheetProps } from '../components/export-sheet.ts'
import { KvOnboardingOverlay } from '../components/onboarding-overlay.ts'
import { KvPlaybackOverlay } from '../components/playback-overlay.ts'
import { KvRecordScreen, type ToastAction } from '../components/record-screen.ts'
import { KvRestoreSheet } from '../components/sheets.ts'
...
type ExportStatus = 'exporting' | 'ready' | 'error'
...
interface ProjectPageEls {
  root: HTMLDivElement
  screen: KvRecordScreen | KvEditorScreen | null
  screenTag: 'kv-record-screen' | 'kv-editor-screen' | null
  playback: KvPlaybackOverlay | null
  exportOverlay: KvExportOverlay | null
  exportSheet: KvExportSheet | null
  restoreSheet: KvRestoreSheet | null
  onboarding: KvOnboardingOverlay | null
  toastEl: HTMLDivElement | null
}

export class KvProjectPage extends KvElement<void> {
  camera: Camera = createCamera(() => this.#syncScreenProps())

  data: ProjectLoaderData | null = null
  /** The projectId the current `data` was loaded for. */
  loadedForId: string | null = null
  #projectId = ''

  mode: ProjectMode = 'record'
  onboardingOpen = false
  onboardingInitialized = false
  playing = false
  toast: ToastState | null = null
  toastTimer = 0
  exportState: ExportUiState | null = null
  restoring = false
  exportRun = 0
  loadVersion = 0
  ...
  bindPreviewCanvas = (element: HTMLCanvasElement | null): void => {
    this.previewCanvas = element
  }

  ensureProjectPromise: Promise<ProjectId> | null = null
  /** Set once the lazy project persists (see original's lazy-create alias). */
  createdProjectId: ProjectId | null = null

  els = {} as ProjectPageEls

  set projectId(next: string) {
    this.#projectId = next
  ...
  setExportState(next: ExportUiState | null): void {
    this.exportState = next
    this.syncView()
  }

  startExport(options?: { force?: boolean }): void {
    if (!this.data) return
    ...
    void (async () => {
      try {
        ...
        if (!options?.force && project) {
          ...
          if (this.exportRun !== runId) return
          if (recovered) {
            this.setExportState({
              status: 'ready',
              progress: 1,
        ...
        if (project) {
          void persistLastExport({ projectId: project.id, result, signature, watermarked }).catch(
            () => undefined,
          )
        }
        if (this.exportRun !== runId) return
  ...
  syncView(): void {
    ...
    if (!project || data.error) {
      ...
      els.root.replaceChildren(
        h('div', { className: 'error-banner' }, data.error ?? 'Project missing'),
        h(
          'div',
          { style: { marginTop: '16px' } },
          h('a', { className: 'btn btn-secondary', href: '/' }, 'Back home'),
        ),
      )
      return
    }
    ...
    const openExportState =
      this.exportState && this.exportState.status !== 'exporting' ? this.exportState : null
    if (openExportState) {
      const exportState = openExportState
      const sheetProps: ExportSheetProps = {
        status: exportState.status as 'ready' | 'error',
        ...
        onShare: () => {
          ...
          void shareFile(result.blob, exportFilename)
            .then((outcome) => {
              ...
              if (outcome === 'shared') this.setExportNotice('Shared!')
            })
            .catch(() => {
    ...
    if (this.toast) {
      ...
      const content: HTMLElement[] = [h('span', null, toast.message)]
