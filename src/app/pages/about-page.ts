...
import { define, fromHtml, h, KvElement } from '../dom.ts'
import { checkForUpdates } from '../lib/app-update.ts'
import { buildDateLabel, shortVersion } from '../lib/build-info.ts'
import { reportError } from '../lib/error-reporting.ts'
import { clearExportCache, estimateExportCacheBytes } from '../lib/export/export-cache.ts'
import { listRearCameras } from '../lib/media.ts'
import { estimateStorageSpace, formatBytes, type StorageSpace } from '../lib/storage-space.ts'
import { brandMark } from '../components/brand-mark.ts'
import { iconBack } from '../components/icons.ts'
...
interface DeviceWithCapabilities extends MediaDeviceInfo {
  getCapabilities?: () => MediaTrackCapabilities & { zoom?: { min?: number; max?: number } }
}

export class KvAboutPage extends KvElement<void> {
  data: { storage: StorageSpace | null; exportCacheBytes: number } = {
    storage: null,
    exportCacheBytes: 0,
  }
   ...
   */
  async onInspectCameras(): Promise<void> {
    if (this.inspectingCameras) return
    ...
    try {
      ...
      for (const device of devices) {
        ...
        const facing = (device as DeviceWithCapabilities).getCapabilities?.()?.facingMode
  ...
  onCheckForUpdates(): void {
    if (this.updateStatus === 'checking' || this.updateStatus === 'updating') return
    ...
    this.replaceChildren(
      h(
        ...
        h(
          ...
          h(
            'div',
            { className: 'about-hero', 'aria-hidden': 'true' },
            brandMark({ size: 96, className: 'brand-hero-art', variant: 'icon' }),
          ),
          h('h1', null, 'Kody ', h('span', null, 'Video')),
          prose,
          h(
            'section',
            ...
            h(
              ...
              'Your recordings are the big consumer — delete old projects from the home screen ' +
                ...
                'so tapping Go on an unchanged project is instant.',
            ),
            h(
              'p',
              null,
              'Cached export files: ',
              h('strong', null, formatBytes(exportCacheBytes)),
              exportCacheBytes > 0
                ? [
                    ' · ',
                    h(
            ...
            this.cacheStatus ? h('p', { role: 'status', 'aria-live': 'polite' }, this.cacheStatus) : null,
          ),
          h(
            'section',
            { className: 'about-section' },
            ...
            h(
              'p',
              null,
              'Wondering why a lens or zoom level isn’t available? Browsers expose cameras very ' +
                'differently across phones — ',
              h(
                'button',
                {
                  type: 'button',
                ...
                this.inspectingCameras ? 'Inspecting…' : 'Inspect cameras',
              ),
              ' shows exactly what this browser reports (nothing is sent anywhere — attach it to a ' +
                'bug report if something looks wrong).',
            ),
            this.cameraReport ? h('pre', { className: 'camera-report' }, this.cameraReport) : null,
          ),
          h(
            'section',
            { className: 'about-section' },
            ...
            h(
              ...
              h(
                'a',
                { href: reportProblemUrl(), target: '_blank', rel: 'noreferrer noopener' },
                'open an issue on GitHub',
              ),
              ' — the link pre-fills your device details so you only have to describe what went ' +
                'wrong. Prefer email (or need help with a purchase)? Write to ',
              h('a', { href: 'mailto:team@kody.video' }, 'team@kody.video'),
              '.',
            ),
          ),
          h(
            ...
            this.updateStatus !== 'idle'
              ? h(
                  ...
                  UPDATE_STATUS_LABEL[this.updateStatus as Exclude<UpdateStatus, 'idle'>],
                )
              : null,
          ),
          h(
