 ...
 * one backend surface — Stripe Checkout verification for the "Remove
 * Watermark" purchase. It never stores anything and never sees media; it
 * just asks Stripe whether the session really completed (including 100%-off
 * promo checkouts).
 *
 * Requires the STRIPE_SECRET_KEY secret on the worker (a restricted key
...
async function verifyPurchase(request, env) {
    ...
    return json({ unlocked: false, error: 'This checkout session is not a completed watermark purchase.' }, 403);
}
export default {
    async fetch(request, env) {
        ...
        // Everything else is a static asset (the assets config also routes most
        // requests there before the worker even runs).
